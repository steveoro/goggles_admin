...
DESC
  task :swimmer_find_by_name do |t|
    ...
    swimmers_found.sort{ |a,b| a.complete_name <=> b.complete_name }.each do |swimmer|
      associated = swimmer.associated_user ? (' => ' + swimmer.associated_user.to_s) : '' 
      logger.info( "\r\n#{swimmer.id} - #{swimmer.complete_name} (#{swimmer.last_name} #{swimmer.first_name}) -#{swimmer.gender_type.code}- #{swimmer.year_of_birth} #{'guessed!' if swimmer.is_year_guessed}#{associated}" )
      logger.info( "<------------------------------------------------------------>" )
      swimmer.badges.sort_by_season('ASC').each do |badge|
        logger.info( " - #{badge.season_id} #{badge.team.name} - #{badge.category_type.code} (#{badge.meeting_individual_results.count})" )
      end
      logger.info( "\r\n\r\n" )
    end
