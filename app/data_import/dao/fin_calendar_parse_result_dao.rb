...
class FinCalendarParseResultDAO
  ...
  class EventTypeDAO
    ...
    def initialize( stroke_type, is_a_relay, is_mixed_gender, relay_phases, length_in_meters )
      raise ArgumentError.new('stroke_type must be a valid StrokeType!') unless stroke_type.instance_of?( StrokeType )
      raise ArgumentError.new('relay_phases and length_in_meters must be valid numbers!') unless relay_phases.to_i > 0 && length_in_meters.to_i > 0
    ...
    alias_method :get_full_name, :to_s
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  def to_s
    "[n.#{ session_order } #{ date_day_token }/#{ date_month_token } @#{ start_time_token }, W:#{ warmup_time_token.present? ? warmup_time_token : ''}]: #{ event_tokens.join(', ') }, parsed events: #{ meeting_events.count }#{ pool_override_text.present? ? ' - '+pool_override_text : '' }"
  end
  #-- -------------------------------------------------------------------------
  #++
end
