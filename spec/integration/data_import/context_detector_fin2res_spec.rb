...
describe "ContextDetector set for 'FIN2res' file types,", type: :integration do
  ...
  class DummyWrapperForFIN2ResContextDectSpecs; include Fin2ResultConsts; end
  let( :dummy_wrapper2 ) { DummyWrapperForFIN2ResContextDectSpecs.new }
  #-- -------------------------------------------------------------------------
  #++
  ...
  context "when parsing category_header," do
    subject { ContextDetector.new( dummy_wrapper2.context_type_category_header, nil ) }
...
#          '',
  ...
  context "when parsing relay_header," do
    subject { ContextDetector.new( dummy_wrapper2.context_type_relay_header, nil ) }
...
#          '',
    ...
    it "doesn't recognize a false-positive in the event index (sample #1)" do
      check_for_parsing_fail(
        [
          '',
          '200 SL, 100 DO, 100 RA, 100 FA, 100 SL, 400 MI, 200 RA, 50 FA, 50 DO, 50 SL, 50 RA, Staffetta 4x50 SL'
        ]
      )
    end

    it "doesn't recognize a false-positive in the event index (sample #2)" do
      check_for_parsing_fail(
        [
          '200 SL, 100 DO, 100 RA, 100 FA, 100 SL, 400 MI, 200 RA, 50 FA, 50 DO, 50 SL, 50 RA, Staffetta 4x50 SL',
          ''
