...
class Fin2ResultDefs < TxtResultDefs
  ...
  def initialize( logger = nil )
    ...
    @context_types = {                                # HEADER CONTEXT(s) def. arrays:
      ...
      stats_details_2b: ContextDetector.new( context_type_stats_details_type2b, logger )
    }
    ...
    @tokenizer_types = {
      ...
      category_header: [                              # 2 row-type conditions => 3 cached rows => the tokenizer list must have 2 elements
        nil,
      ...
      relay_header: [                                 # 2 row-type conditions => 3 cached rows => the tokenizer list must have 2 elements
        nil,
        [
    ...
    @tokenizer_fields = {
      ...
      relay_row: [
        [ :result_position, :team_name, :category, :result_time, :result_score ]
      ],
    ...
    @context_keys = {
      ...
      category_header: [:distance, :style, :gender],
      relay_header:      [:type]                     # (type includes also the gender token)
    }
  ...
  def describe
    'FIN2 format - Tuscany regional FIN format'
  end
end
