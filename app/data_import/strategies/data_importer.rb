  ...
  def phase_1_parse()
    ...
    unless @data_import_session.file_format
      ...
      when /sta|stl/
        ...
        @data_import_session.file_format = 'results?'
      end
      @data_import_session.save!
    end
    ...
    append_to_log_file( @data_import_session, "\r\nFile format detectd: #{@result_hash[:parsing_defs].describe}" )
  ...
  def phase_1_2_serialize()
    ...
    if is_ok && @season                             # -- MEETING (digest/serialization) --
      meeting_builder = DataImportMeetingBuilder.build_from_parameters(
        ...
        meeting_header_row,
        meeting_dates,
        false # [Steve, 20170306] THIS SHOULD BE ALWAYS DISABLED
      )
...
# DEBUG
  ...
  def phase_3_commit()
    ...
    update_meeting_flags( @meeting ) if is_ok && @meeting
                                  ...
                                  # *** FIX/Re-esteem committed meeting program's begin times:
    # Do not esteem possible starting times if we don't met the optimal conditions:
    # - we should have a meeting defined
    # - everything should be ok
    # - no separate-category starting lists for any events
    unless (
        !@meeting.instance_of?( Meeting ) ||
        !is_ok ||
        @meeting.meeting_events.any?{ |me| me.has_separate_category_start_list? }
    )
      is_begin_time_fixed = BeginTimeCalculator.compute_for_all( @meeting, sql_diff_text_log )
      append_to_log_file(
        @data_import_session,
    ...
    if @meeting && is_ok
      ...
      @data_import_session.sql_diff = @meeting.id.to_s
      @data_import_session.save!
    end
    is_ok
  end
