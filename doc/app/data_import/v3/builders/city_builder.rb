...
=end
class V3::CityBuilder < V3::EntityBuilder

  # Searches for an existing City assuming city name can be deduced
  # by the given Team name.
  #
  # The current implementation uses V3::CityComparator to detect the matching city
...
# DEBUG
#        puts "\r\nSearching: #{team_name}"
        city = V3::CityComparator.new.search_composed_name( team_name )
        set_result( city )
      end
...
#        puts "Primary search failure. Searching on secondary entity..."
        city = V3::CityComparator.new.search_composed_name( team_name, DataImportCity )
        set_result( city )
      end
