...
require_relative './data_import_swimmer_builder'
require_relative './data_import_badge_builder'
    ...
    self.build( data_import_session ) do
      ...
      set_up do
        ...
        if detail_row.instance_of?( Hash )
          ...
          athlete_badge = detail_row[:fields][:swimmer_code].to_s.size < 1 ? ( detail_row[:fields][:team_code].to_s.size < 1 ? '?' : detail_row[:fields][:team_code] ) : detail_row[:fields][:swimmer_code]
