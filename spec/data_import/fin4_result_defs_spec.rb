...
require_relative '../../app/data_import/fin4_result_defs'
#require_relative '../../lib/framework/console_logger'


describe Fin4ResultDefs, type: :model do

  context "for a well-defined instance," do

    subject { Fin4ResultDefs.new( ConsoleLogger.new ) }
    ...
    describe "#context_types_children_of()" do
      ...
      context "when called for :individual_category_header," do
        it "has a only :result_row key" do
          expect( subject.context_types_children_of(:individual_category_header).keys ).to contain_exactly( :result_row )
        end
      end
      context "when called for :relay_category_header," do
        it "has a only :result_row key" do
          expect( subject.context_types_children_of(:relay_category_header).keys ).to contain_exactly( :relay_row )
        end
      end
