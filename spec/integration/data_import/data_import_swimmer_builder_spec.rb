...
#      puts "- MIR/PRG:      #{mir.meeting_program.inspect}"
  ...
  # Existing or matching fixture params:
  let(:di_swimmer) do
    swimmer_year = category_type.season.begin_date.year - category_type.age_end + 2
    create(
      :data_import_swimmer,
  ...
  # Should not create a secondary entity row if there's a match w/ an existing name and category range:
  context "after a self.build(birthyear: nil, category: given) with a matching secondary entity row," do
    subject do
# DEBUG
#      puts "\r\n\r\n**** DataImportSwimmerBuilder spec, line 259... ***"
#      puts "- DataImportSwimmer.count before params....: #{ DataImportSwimmer.count }"
#      puts "- category_type.season.begin_date.year.....: #{ category_type.season.begin_date.year }"
#      puts "- category_type.age_end....................: #{ category_type.age_end }"
#      puts "- category_type............................: #{category_type.inspect}"
#      puts "- di_swimmer...............................: #{di_swimmer.inspect}"
#      puts "- data_import_sessios......................: #{data_import_session.inspect}"
#      puts "- DataImportSwimmer.count before build.....: #{ DataImportSwimmer.count }"
      builder = DataImportSwimmerBuilder.build_from_parameters(
        data_import_session,
...
# DEBUG
#      puts "\r\n- DataImportSwimmer.count after: #{ DataImportSwimmer.count }"
      builder
    end
    it_behaves_like "DataImportSwimmerBuilder with a matching secondary entity row"
  end
...
#      puts "- swimmer_2: #{swimmer_2.inspect}"
    ...
    it "does not create any additional primary entity row" do
      # (+2 only from the factory creation in the subject: 1 from the MIR and 1 from the swimmer_2)
    ...
    describe "#result_row" do
      ...
      it "has the ID of the matching row, complying with the parameters" do
        expect( subject.result_row.id ).to eq( swimmer_2.id )
      end
    end
    describe "#result_id" do
      it "returns a negative ID (the primary search is successful)" do
