...
class GeocodingParser
  ...
  def initialize( verbose_address_text )
    raise ArgumentError.new('The verbose_address_text must be a non-empty String.') unless verbose_address_text.instance_of?( String ) && verbose_address_text.size > 0
  ...
  def stub_api_request( text_response )
    ...
    if @status != 'OK'
      @text_log << "GeoCoder Maps API --FAILED-- with result status '#{status}'.\r\n"
    else
      @text_log << "GeoCoder Maps API OK. Valid JSON result obtained.\r\n"
    end
    @json_response
  end
  #-- -------------------------------------------------------------------------
