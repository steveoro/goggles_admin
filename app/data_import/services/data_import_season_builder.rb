...
#    puts "\r\nSeason -- build_from_parameters: #{header_date}, season_type: #{season_type_id}, edition: #{edition}"
    self.build( data_import_session ) do
      ...
      set_up do                                   # Set the fields:
        ...
        @begin_date     = "#{year}-09-01"     # This is also used as start date for the search
        @end_date       = "#{year + 1}-09-01"
        @end_search     = "#{year + 1}-10-30" # End search date must be greater than actual start date of the next season
      ...
      search do
        primary    [
          ...
          @season_type_id, @begin_date, @end_search
        ]
        secondary  [
          "(data_import_session_id = ?) AND (season_type_id = ?) AND (begin_date >= ?) AND (end_date <= ?)",
          data_import_session.id, @season_type_id, @begin_date, @end_search
        ]
        default_search
      end
