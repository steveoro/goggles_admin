    ...
    self.build( data_import_session ) do
      ...
      set_up do
        ...
        if splitted_name.size == 2                      # Use last & first name only when the splitting is certain
          ...
          if splitted_name.size == 3 && splitted_name[0].length <= 5   # Leega: Some italian common "composite" last names prefix
            common_composite_last_name_prefix = ['DE', 'DI', 'DEGLI', 'DAL', 'LA', 'LE', 'LO']
            if common_composite_last_name_prefix.include?( splitted_name[0] )
              @last_name  = splitted_name[0] + ' ' + splitted_name[1]
      ...
      search do
        if @is_year_guessed
          ...
          swimmer_year = category_type.season.end_date.year - category_type.age_end
...
# DEBUG
          ...
          # instance:
          analyzer = SwimmerNameAnalyzer.new( data_import_session )
...
#          analyzer.swimmers.each{ |row| puts "\r\n- id: #{row.id}, #{row.complete_name}, gender: #{row.gender_type_id}, #{row.year_of_birth}"}
          result = analyzer.analyze(
              @complete_name, swimmer_year, gender_type.id,
              @max_year, category_type, 0.99, 0.8
          )
# DEBUG
#          puts analysis_log
#          puts "\r\n#{ result }"
