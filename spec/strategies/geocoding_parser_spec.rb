...
describe GeocodingParser, type: :strategy do
  ...
  before(:all) do
    @json_fixture_1 = File.open( File.join( Rails.root, 'spec/fixtures/samples/maps_API_geocoding_response_fixture_01.json' ) ).read
    @json_fixture_2 = File.open( File.join( Rails.root, 'spec/fixtures/samples/maps_API_geocoding_response_fixture_02.json' ) ).read
    @json_fixture_3 = File.open( File.join( Rails.root, 'spec/fixtures/samples/maps_API_geocoding_response_fixture_03.json' ) ).read
  end
  ...
  let( :json_fixture_3_query_address )  { "piscina comunale, via Melato, Reggio Emilia" }
  #-- -------------------------------------------------------------------------
  #++


  context "with valid parameters," do
    subject { GeocodingParser.new( json_fixture_3_query_address ) }
    ...
    describe "#stub_api_request" do
      ...
      context "for a valid JSON text (real case fixture #3, w/ OK status)," do
        it "is an Hash" do
          expect( subject.stub_api_request( @json_fixture_3 ) ).to be_an( Hash )
        end

        it_behaves_like( "well-structured JSON Geocoding API response", :use_fixture_3 )

        it "has the expected formatted_address" do
          hash_result = subject.stub_api_request( @json_fixture_3 )
          expect(
            hash_result["results"].first["formatted_address"]
          ).to eq( "Via Melato, 2/D, 42122 Reggio Emilia RE, Italy" )
        end
        it "adds 'API OK' as last line of the #text_log" do
          subject.stub_api_request( @json_fixture_3 )
          expect( subject.text_log.split("\r\n").last ).to include( "API OK" )
        end
        it "resets #is_processed to false" do
          subject.stub_api_request( @json_fixture_3 )
    ...
    describe "#extract_data!" do
      ...
      context "for a valid JSON text (real case fixture #3, w/ OK status)," do
        before(:each) do
          expect( @json_fixture_3 ).to be_a( String )
          subject.stub_api_request( @json_fixture_3 )
          expect( subject.json_response ).to be_an( Hash )
          subject.extract_data!
        end
