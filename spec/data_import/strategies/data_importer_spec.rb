...
describe DataImporter, type: :strategy do
  ...
  context "for a new empty instance," do
    subject { DataImporter.new( 1, existing_data_import_session ) }

    it_behaves_like( "(the existance of a method)", [
      :flash, :data_import_session,
      :header_fields_dao,
      :meeting,
      :result_hash,
    ...
    describe "#initialize" do
      ...
      it "creates a new DataImportSession row when not provided" do
        expect{ DataImporter.new(1) }.to change{ DataImportSession.count }.by(1)
      end
    ...
    describe "#try_detect_season_from_header_fields" do
      it "returns the current @season instance when already set" do
        importer = DataImporter.new( 1, existing_data_import_session )
...
#        puts "\r\n- #{importer.header_fields_dao.inspect}"
    ...
    describe "#phase_1_parse" do
      ...
      before( :all ) do
        ...
        @phase_1_subject = DataImporter.new( 1, @phase_1_session )
...
# XXX [Steve, 20170428] MEETING CREATION DURING DATA-IMPORT IS NOT SUPPORTED ANYMORE!
...
#
    ...
    describe "#phase_3_commit" do
      ...
      #  by FactoryBot at the end of each RSpec example.)
    end
    #-- -----------------------------------------------------------------------
    #++


    describe "#update_meeting_flag" do
      let(:data_import_session) { create( :data_import_session ) }
      let(:meeting) { Meeting.all.limit(1000).sort{0.5 - rand}.first }
      subject do
        DataImporter.new( 1, data_import_session )
      end

      context "with a nil meeting parameter," do
        it "returns nil" do
          expect( subject.update_meeting_flag( nil, 'are_results_acquired', false ) ).to be nil
        end
      end

      context "with non-Meeting parameter," do
        it "returns nil" do
          expect( subject.update_meeting_flag( 'a string but not a meeting', 'are_results_acquired', false ) ).to be nil
        end
      end

      context "with an existing and valid Meeting parameter," do
        ['are_results_acquired', 'has_start_list'].each do |bool_flag_name|
          it "returns the meeting instance" do
            expect( subject.update_meeting_flag( meeting, bool_flag_name, false ) ).to eq( meeting )
          end
          it "sets the specified meeting flag to the chosen value" do
            subject.update_meeting_flag( meeting, bool_flag_name, false )
            expect( meeting.send(bool_flag_name) ).to be false
          end
          it "updates the #sql_diff_text_log with the corresponding UPDATE statement" do
            subject.update_meeting_flag( meeting, bool_flag_name, false )
            expect( subject.sql_diff_text_log ).to include(
              "UPDATE meetings SET #{ bool_flag_name } = '0' WHERE id = #{meeting.id};\r\n"
            )
          end
        end
      end
    ...
    describe "#update_meeting_flags" do
      ...
      context "with a nil meeting parameter," do
        it "returns nil" do
          expect( subject.update_meeting_flags(nil) ).to be nil
        end
      end

      context "with non-Meeting parameter," do
        it "returns nil" do
          expect( subject.update_meeting_flags('a string but not a meeting') ).to be nil
        end
      end

      context "with an existing and valid Meeting parameter," do
        it "sets the data-import meeting flags to the correct value" do
          subject.update_meeting_flags( meeting )
          expect( meeting.are_results_acquired ).to eq( meeting.meeting_individual_results.count > 0 )
          expect( meeting.has_start_list ).to eq( meeting.meeting_entries.count > 0 )
        end
        it "updates the #sql_diff_text_log with the corresponding UPDATE statement" do
          subject.update_meeting_flags( meeting )
          are_results_acquired = meeting.meeting_individual_results.count > 0
          has_start_list = meeting.meeting_entries.count > 0
          expect( subject.sql_diff_text_log ).to include(
