...
require_relative '../../../app/data_import/fin3_result_defs'
require_relative '../../../app/data_import/uisp_result_defs'
...
describe FileFormatParser, type: :strategy do
  ...
  context "with a FIN2 result file type," do
    [
      ...
      File.join(Rails.root, 'spec/fixtures/ris/ris20170326livornohardswim-sample.txt')
    ].each do |filename|
      describe "#parse" do
        it "returns the correct constant associated with the file type for #{File.basename(filename)}" do
          format_parser = FileFormatParser.new( filename )
  ...
  context "with a UISP result file type," do
    [
      File.join(Rails.root, 'spec/fixtures/ris/ris20170205uispregemilia.txt')
    ].each do |filename|
      describe "#parse" do
        it "returns the correct constant associated with the file type for #{File.basename(filename)}" do
          format_parser = FileFormatParser.new( filename )
          expect( format_parser.parse ).to be_an_instance_of( UispResultDefs )
        end
      end
    end
  end
