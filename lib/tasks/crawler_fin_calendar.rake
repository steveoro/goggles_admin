...
namespace :crawler do
  ...
  desc <<-DESC
  *** FIN Calendar Synch STEP 1: "Calendar refresh" ***
...
Apifier's' API used to retrieve the list of all sub-pages for each year:

  - Supermaster_FIN_current_season_meetings / Supermaster_FIN_season_XX2_calendar:
    Defined at https://www.apifier.com/crawlers/p2DPKSzRAZWavEH5D;
    Crawls the current calendar page stored at:
    https://www.federnuoto.it/discipline/master/circuito-supermaster.html
    or
    https://www.federnuoto.it/discipline/master/circuito-supermaster/stagione-20xx-20xx.html
...
DESC
  task( fin_calendar_step1: ['require_env'] ) do |t|
    ...
    # [Steve, updated 20171116]
    # To change or update the source URL links, edit directly the crawlers at:
    # https://my.apify.com/crawlers
    #
    # Check the advanced settings for the crawler to have:
    # - a shader IP
    ...
    api_run_endpoint = if season_id > 172
      ...
      "https://api.apify.com/v1/YZw3JnXkocmreiBvj/crawlers/kYnFnrz3yWwLK33of/execute?token=9mRSNT5qvYrtQabf3vrhf3ahp"
    elsif season_id == 142
      "https://api.apify.com/v1/YZw3JnXkocmreiBvj/crawlers/8oQcDC2BDNXp7notT/execute?token=EQgNn8zFgL5joLFdQhZoKfFtu"
    elsif season_id == 152
  ...
  desc <<-DESC
  *** FIN Calendar Synch STEP 2: "Data extraction" ***
...
Options: [ [season_id=Season_id|<#{DEFAULT_SEASON_ID}>] | [row_id=FIN_Calendar_row_id] ]
         [user_id=<1>]
...
DESC
  task( fin_calendar_step3: ['require_env'] ) do |t|
    ...
    full_action_log_pathname = File.join( output_path, "#{ DateTime.now().strftime('%Y%m%d%H%M') }#{ base_name }.log" )
...
#    puts "------------------------------------------------------------------\r\n"
  ...
  # Executes the crawler defined at the specified API endpoint, retrieves the results,
  # parses them into a dedicated struct, and serializes the updated calendar.
  #
  # === Sample JSON result (that will be parsed):
  #
  #   [{
  #     "year": "2016",
  #     "month": "Ottobre",
  #     "days": "23",
  #     "city": "Milano",
  #     "description": "Distanze speciali Lombardia",
  #     "link": "/discipline/master/circuito-supermaster/815.html?view=manifestazione",
  #     "url": "http://www.federnuoto.it/discipline/master/circuito-supermaster.html"
  #   },
  #   {
  #     "year": "2016",
  #     "month": "Ottobre",
  ...
  #     "link": "/discipline/master/circuito-supermaster.html?view=risultati&codice=A23A&anno=2017",
  #     "url": "http://www.federnuoto.it/discipline/master/circuito-supermaster.html"
  #   },
  #   # [...]
  #   ]
  ...
  #
  def scan_apifier_crawler_results_for_calendar( api_run_endpoint )
...
#    puts "json_result.first => #{ json_result.first.class }\r\n"
  ...
  def compose_db_diff_filename( output_path, qualifier_name, code )
    file_name = "#{ DateTime.now().strftime('%Y%m%d%H%M') }#{ qualifier_name }_#{ code }.diff.sql"
