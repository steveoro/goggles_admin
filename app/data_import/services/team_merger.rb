...
class TeamMerger
  ...
  def collect_phase()
    ...
    process_text_log << "\r\n"
                                                    ...
                                                    # ===  MeetingReservation (Ref.: team) ===
    collector = EntityRowDupCollector.new( MeetingReservation )
    collector.process( @slave_team.id, team_filter, @master_team.id, team_filter ) do |src_row, dest_row|
      (dest_row.team_id == src_row.team_id) &&
      (dest_row.swimmer_id == src_row.swimmer_id) &&
      (dest_row.meeting_id == src_row.meeting_id)
    end
    @row_collectors[ MeetingReservation.table_name ] = collector
    log_row_sizes( MeetingReservation, :inspect )
    process_text_log << "\r\n"

                                                    # ===  MeetingEventReservation (Ref.: team) ===
    collector = EntityRowDupCollector.new( MeetingEventReservation )
    collector.process( @slave_team.id, team_filter, @master_team.id, team_filter ) do |src_row, dest_row|
      (dest_row.team_id == src_row.team_id) &&
      (dest_row.swimmer_id == src_row.swimmer_id) &&
      (dest_row.meeting_event_id == src_row.meeting_event_id)
    end
    @row_collectors[ MeetingEventReservation.table_name ] = collector
    log_row_sizes( MeetingEventReservation, :inspect )
    process_text_log << "\r\n"

                                                    # ===  MeetingRelayReservation (Ref.: team) ===
    collector = EntityRowDupCollector.new( MeetingRelayReservation )
    collector.process( @slave_team.id, team_filter, @master_team.id, team_filter ) do |src_row, dest_row|
      (dest_row.team_id == src_row.team_id) &&
      (dest_row.swimmer_id == src_row.swimmer_id) &&
      (dest_row.meeting_event_id == src_row.meeting_event_id)
    end
    @row_collectors[ MeetingRelayReservation.table_name ] = collector
    log_row_sizes( MeetingRelayReservation, :inspect )
    process_text_log << "\r\n"
  ...
  def update_phase()
    ...
    execute_update( MeetingReservation ) &&
    execute_update( MeetingEventReservation ) &&
    execute_update( MeetingRelayReservation ) &&
  ...
  def delete_phase()
    ...
    execute_delete( MeetingReservation ) &&
    execute_delete( MeetingEventReservation ) &&
    execute_delete( MeetingRelayReservation ) &&
