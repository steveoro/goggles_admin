...
class ParseResultConverter
  ...
  def to_parse_result( source_parse_result_hash, source_parsing_defs, season )
    ...
    elsif source_parsing_defs.instance_of?( UispResultDefs )
      adjust_values_from_uisp_parsing( parse_result_hash, season )
  ...
  def rebuild_category_headers( parse_result_hash )
    ...
    parse_result_hash[:result_row].each do |result_row|
      ...
      old_header_row  = parse_result_hash[:category_header].find({}) do |category_header|
        result_row[:id] == category_header[:id]
      end
      ...
      result_row[:fields][:swimmer_year] = convert_yy_2_yyyy( birth_year )
      ...
      parse_result_hash[:category_header].delete_if do |category_header|
        # We must remove all headers that do not have result rows associated to them:
        parse_result_hash[:result_row].find(){ |row| row[:id] == category_header[:id] }.nil?
      end
    end
  end


  # Scans and updates the whole parse result hash adapting some of its values to
  # the "standardized" format & values expected by the entity builders.
  #
  def adjust_values_from_uisp_parsing( parse_result_hash, season )
    # Adjust all result row non-standard tokens:
    parse_result_hash[:result_row].each do |result_row|
      result_row[:fields][:swimmer_year] = convert_yy_2_yyyy( result_row[:fields][:swimmer_year] )
    end

    # Leega: Should be not needed.
    # Event code will be used to find out gender, category and stroke
    #
    # Adjust all relay headers non-standard tokens:
    # - gender forced to verbose (localized) description
    # - category forced to standard code
    # - type forced with prefix 'staffetta' or 'mistaffetta'
    parse_result_hash[:relay_header].each do |relay_header|
      event_uisp_code = relay_header[:fields][:event_uisp_code]

      gender_type_desc = GenderType.retrieve_description_by_uisp_code( event_uisp_code[0] )
      relay_header[:fields][:gender] = gender_type_desc if gender_type_desc && gender_type_desc != '?'

      if event_uisp_code[0] && (event_uisp_code[0].upcase == 'M' || event_uisp_code[0].upcase == 'F')
        relay_header[:fields][:type] = 'staffetta ' + relay_header[:fields][:type]
      else
        relay_header[:fields][:type] = 'mistaffetta ' + relay_header[:fields][:type]
      end

      category_type = CategoryType.where([
            '(season_id = ?) AND (federation_code = ?)',
            season.id, relay_header[:fields][:category]
          ]).last
      # Set the expected code for the category:
      relay_header[:fields][:category] = category_type.code if category_type
    end
  end


  # Tries to give the correct 4-digit year to 2-digit year birth dates.
  #
