...
Resulting log files are stored into '#{LOG_DIR}'.
  ...
  season=season_id [log_dir=<#{LOG_DIR}>]
...
DESC
  task :season_swimmer_last_best do |t|
    ...
    events_to_scan  = []
    involved_events = season.event_types.uniq.map{ |et| et.code }
    ...
    involved_swimmers = 0
    season_type.swimmers.has_results.distinct.sort_by_gender_type( 'ASC' ).each do |swimmer|
...
Resulting log files are stored into '#{LOG_DIR}'.
  ...
  meeting=meeting_id
  [persist=<false>] [recalculate=<false>] [log_dir=<#{LOG_DIR}>]
...
DESC
  task :meeting_swimmer_last_best do |t|
    ...
    # Determinate events to scan for
    events_to_scan  = []
    events_to_scan = meeting.get_events_by_pool_types.reject{ |e| e == nil }.map{ |e| e.get_key }
    logger.info( " - Scan for: #{events_to_scan.size} event types" )
    ...
    csv_rows << headers.join(';')
    involved_swimmers = 0
    meeting.swimmers.has_results.distinct.sort_by_gender_type( 'ASC' ).each do |swimmer|
    #meeting.meeting_entries.select(:swimmer_id).distinct.map{ |me| me.swimmer_id }.each do |swimmer_id|  # If only start-list present
    #  swimmer = Swimmer.find(swimmer_id)
      ...
      if involved_seasons.count > 0
        ...
        events_to_scan.each do |event_key|
          ...
          if !event_by_pool_type.event_type.is_a_relay
            best_swam = swimmer_best_finder.get_involved_season_last_best_for_key( involved_seasons, event_key )
            if best_swam
